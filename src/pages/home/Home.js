...
const Home = () => {
  ...
  return (
    <div className="home-container p-5" style={style}>
                  ...
                  movies.results.map((item)=>{
                    ...
                    <div className="card" key={item.id}>
                        ...
                        <Link className="" to={"/" + item.id} style={{textDecoration:"none"}}>
                            <button className="btn btn-success" onClick={()=>{saveDataAboutJob(item.id , item.position ,item.offer ,item.description )}}>Show&Apply</button>
                          </Link>
                    </div>
                    )
                  })
